...
- 如果你在**某个项目里工作**:执行任务前先过 **`A2-R2`(入口)+ `A2-R3`(对应表)**;收尾时过 **`A2-R4`(哨兵)**。
  ⚠️ 但**收尾哨兵不能替代左移门** —— 你的表里凡写着"**在某事之前**必须先调用 X"的行,
  逐个问"**这类时机有门吗**":见 **`A2-R6`** 的「覆盖面自检」。
...
| **skill 痕迹门** | 同上 | 改了实现代码,但**本会话** transcript 里没有该调用的 skill → **拒绝提交**;豁免 `[no-skill]` | ⚠️ **半硬 —— 判据是<u>会话级</u>:一次调用后对本会话<u>永久满足</u>(见 `A2-R15`)**。抽取事实而非自述这部分仍是硬的(见 `A2-R9`),但**"本次改动前有没有调"它判不了** |
...
⚠️ **"动手那一刻"不止一种,而且每种要各有各的门**:
「**要改实现文件时**」是一种(对应 TDD);「**遇到 bug / 异常、准备提出修法之前**」是另一种。
⚠️ **收尾哨兵(见 `A2-R4`)不能替代左移门** —— 它抓得住「**漏调的记录**」,
抓不住「**漏调本身**」:事后再提醒,那个判断**已经在错的前提下做完了**。
...
**反例(实例 · 第二类时机)**:一次排查故障的全过程 ——
规则表里明写着「遇到 bug / 测试失败 / 异常 → **在提出修法之前**先调某条排查纪律」,
而**没有任何门在"遇到异常那一刻"触发**。那次排查**做对了**(用线程栈直接定位到真因,并做了单变量对照),
但**那条纪律从头到尾没被调用**;发现它漏了,已经是**在收尾写提交前检查点**的时候。
⇒ 这正是本条的形状:**门只在收尾,所以它抓住的是"记录",不是"调用"**。
按 `A2-R4`,收尾哨兵**按设计工作**;本条要说的是它**覆盖不到哪里**。*来源:见 §9。*
...
**覆盖面自检(照做)**:现有左移门锚在「**要改实现文件**」这一类时机。
⇒ 拿你的规则表过一遍,凡写着"**在某事之前**必须先调用 X"的行,**逐个问**:
**这类时机有左移门吗?还是只有收尾哨兵?**
只有哨兵的,按本条 —— **它还没左移**,而它造成的漏,**事后一定会以"记录"的形态出现,让你误以为机制在工作**。
...
### `A2-R15` · 门禁的判据若锚在「**累计状态**」上,它**一次满足后永久满足**

**规则**:门禁的判据必须锚在「**本次事件**」上(这一次提交 / 这一次改动 / 这一次声明),
**不得锚在「累计状态」**上(本会话出现过 / 曾经调用过 / 历史上有过)。
判据取累计 ⇒ 门在**第一次满足之后就不再会失败** ⇒ 它退化成
**"一个永远不失败的检查"** —— 而它**比没有检查更坏**:它让下游
(锚点 / 复盘 / 签核)**相信"这里有结构兜着"**。

**反例(实例)**:2026-09-15 · 某评测门项目 · 收尾批次。
该仓的 skill 痕迹门(示例,本仓实现:`.githooks/commit-msg`)判据是
「**本会话** transcript 里出现过 `test-driven-development`」—— 而 trace 里那个字段是**会话累计**的。

实测(客观采集,非自述):该会话 `skill_calls = 4`(TDD 只在**会话早期**调了 1 次),
同期 `bash_calls = 208` / `edit_calls = 38`;TDD 那 1 次之后有 **4 个编码任务**
(其中一次**改了判分器输入契约**,属高风险改动)**一次都没调** ——
**而这道门全程一次都没有响过。**

⚠️ **更坏的一点**:**规则文本与实现共享同一个错误假设**。本库 `A2-R5` 的强度表原文写的就是
「…但**本会话** transcript 里没有该调用的 skill → 拒绝提交 | **硬**」——
**「本会话」这个词本身就是洞**。⇒ 修要**两边都修**:实现,以及这张表。

**正例(怎么做到)**:
1. 判据写成「**本次改动之前**」:比较「最后一次该 skill 的调用」与
   「本次提交里**第一个实现改动**的时刻」,前者须落在后者所属的**连续工作段**内;
2. **做不到就如实降级标注**(这是 `A2-R5` 的直接应用,不是例外)——
   标成「硬,**但判据是会话级 ⇒ 一次调用后对本会话永久满足**」;
3. 判定必然带**误拦风险**(同一件工作分多次提交时,"本次改动前"可能落在上一次调用之前)
   ⇒ **必须带显式豁免**(同 `[no-test: <理由>]` 的约定),且**豁免率可统计**。

> **这是 `A2-R5` 的补丁**:凡强度表里标「硬」的,都要回头问一句 ——
> ### **「它会不会一次满足之后永久满足?」**

**适用场景**:任何「门禁 / 守卫 / 检查」的**判据设计**;尤其当判据读的是
**被累计的字段**(会话级计数、历史总数、"曾经发生过")时。

**不适用场景**:本就锚在本次事件上的检查(如"这条命令成不成功""这次测试过没过")——
它们天然不会累计。

**来源**:2026-09-15 · 某评测门项目 · 收尾批次**末**。
触发方式值得记:**不是自查发现的,是业务方追问**「哪些之前用的 skill 现在跳过了」之后,
去采集 trace 才拿到客观数字 —— 在此之前,门一直是绿的。

**怎么验证它被落地**(可证伪):
- **结构性**:构造「会话开头调一次、之后连续改实现」的场景 ⇒ 门**必须仍会拦**;
- **反例守卫**:写一条测试断言「会话早期调用过、但那之后又改了实现」⇒ 门**必须红**;
  (本仓对守卫的一贯要求:先证明它**能红**,再谈它有没有用。)
- **诚实兜底**:若实现暂不能做到,则上面第 2 条的**降级标注**是否已落到强度表里 ——
  **没标 = 仍在说谎**。
...
- 你的**规则表里有"在某事之前必须先调用 X"这类行** → 逐类问"这类时机有门吗"(`A2-R6` 的覆盖面自检)。
...
- 关联:与 `A1` 篇同源(同一命题的第一次发作在"纪律"上、第二次在"技能"上);
  ...
  **子 Agent 与委派**见 `A3` 篇。
- 实例(`A2-R6` · 第二类时机):`某 RAG 服务项目` · 2026-09-17 · 单模块测试闭环轮 ——
  一次排查故障**做对了方法**(用线程栈直接定位真因 + 单变量对照),
  但规则表里那条"**提出修法之前**必须先调"的排查纪律**全程没被调用**;
  是**收尾的提交前检查点**把它抓出来的 —— 即**滞后门抓住了"记录",没抓住"调用"**。
  同轮该检查点还按 `A2-R5` 如实标注了"**命中未调**",没有含糊过去。
...
| **左移覆盖面**(`A2-R6`) | 规则表里凡"**在某事之前**必须先调用 X"的**每一类时机**,都能指出**它有没有左移门**;只有收尾哨兵的,如实记为「**未左移**」——而不是记为"有哨兵兜着" |
...
- **2026-09-16 新增 `A2-R15`,并**如实降级一处强度**。
  来源:2026-09-15 · 某评测门项目 · 收尾批次末,业务方追问「哪些之前用的 skill 现在跳过了」
  ⇒ 采集 trace 得客观数字,发现该仓的 skill 痕迹门**全程一次未响**。
  根因:该门判据读的是**会话累计**字段 ⇒ **一次调用后对本会话永久满足**。
  ⇒ ① 新增 `A2-R15`(门禁判据不得锚在累计状态上);
  ② 按 `A2-R5`「逐条如实标注强度」,把强度表的 **skill 痕迹门** 从「硬」改为
  **「半硬 —— 判据是会话级」**,并在 `A2-R15` 里写明实现改动**必须带测试与豁免**。
  ⚠️ **本条是"规则文本与实现共享同一个错误假设"的实例** ——
  强度表原文写的「但**本会话** transcript 里没有…」,那个「本会话」本身就是洞。
  ⇒ 登记于此,**改文本与改实现是两件事,缺一仍是说谎**。
- 2026-09-17 为 **`A2-R6`** 补第二类时机与覆盖面自检:
  「**动手那一刻**」不止「要改实现文件」一种,「**遇到 bug / 异常、准备提出修法之前**」是另一种;
  **收尾哨兵(`A2-R4`)抓得住"漏调的记录",抓不住"漏调本身"**,两者不可互替。
  来源:`某 RAG 服务项目` · 单模块测试闭环轮 —— 一次排查方法做对了、而那条"提出修法前必须先调"的纪律全程未调,
  是**收尾检查点**把它抓出来的。同批:§0/§8/§9/§10 同步补齐。
  ⚠️ **本条只改文本** —— 是否为"遇到 bug"这一类时机**真装一道左移门**,是另一个决定(动 harness),**不在本次**。
